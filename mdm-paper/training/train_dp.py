...
from polya_mixture.datasets import get_user_counts, make_cifar10_datasets
from polya_mixture.utils import (add_dataset_args, add_experiment_args,
                                 add_mle_args, add_init_algorithm_args,
                                 add_algorithm_args,
                                 add_histogram_algorithm_args,
                                 add_user_visualisation_args)

from polya_mixture.mle import solve_polya_mixture_mle
...
if arguments.precomputed_parameter_filepath is None:
    ...
    save_dir = (
        ...
        f'dp_cifar10_{arguments.num_mixture_components}_mixture_{arguments.dirname}'
    )
...
# Create user dataset length samplers, one for each histogram
def make_len_sampler(p):
    """
    Factory for creating a user dataset len sampler
    from a histogram of user lengths.
    """
...
user_dataset_len_samplers = [
    make_len_sampler(num_samples_distribution)
    for num_samples_distribution in num_samples_distributions
]

# For model training use GPU if available
os.environ['RAMSAY_PYTORCH_DEVICE'] = 'cuda' if torch.cuda.is_available(
) else 'cpu'

# Create simulation data for training using learned parameters and
# length samplers
# dataset type: choices=['artificial_federated_dataset', 'federated_dataset']
print('simulation dataset')

training_federated_dataset, val_federated_dataset, central_val_data = (
    make_cifar10_datasets(dataset_type='federated_dataset',
                          data_dir=arguments.data_dir,
...
# Set to True to generate several cohorts of users and save their label
# counts, will save the counts in a pkl file. Can be used to TSNE
# visualize generated cohorts of users from the three different
# experiment types. Only implemented to run locally.
print('save user datasets locally')
check_users = True
if check_users:
    ...
    timestamp = current_time.strftime("%Y-%m-%d_%H-%M")
    save_filename = (
        #f'cifar10_user_counts_{arguments.num_mixture_components}_mixture_{timestamp}')
        f'cifar10_user_counts_{arguments.num_mixture_components}_mixture')
    save_path = os.path.join(dir_path, f'{save_filename}.joblib')

    get_user_counts(training_federated_dataset, num_classes,
                    arguments.num_iterations_visualization,
                    arguments.cohort_size_visualization, save_path)
