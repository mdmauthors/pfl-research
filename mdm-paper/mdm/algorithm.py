...
class PolyaMixtureAlgorithm(
    ...
    def get_next_central_contexts(
        ...
        if (model.alphas <= 0).any(): 
            raise AssertionError(f'Cannot have zero elements in alphas: {model.alphas}')
        if (model.num_samples_distribution <= 0).any(): 
            raise AssertionError(f'Cannot have zero elements in num_samples_distribution: {model.num_samples_distribution}')
        if (model.phi <= 0).any():
            raise AssertionError(f'Cannot have zero elements in phi: {model.phi}')
    ...
    def simulate_one_user(
        ...
        if (model.alphas <= 0).any():
            raise AssertionError('> 0 alpha params have zero values, which would cause algorithm to fail. Cannot proceed.')

        if (model.phi <= 0).any():
            raise AssertionError('> 0 phi params have zero values, which would cause algorithm to fail. Cannot proceed.')

        if (model.num_samples_distribution <= 0).any():
            raise AssertionError('> 0 num_samples_distribution have zero values, which would cause algorithm to fail. Cannot proceed.')
        ...
        selected_bin = -1
        for i, bin_edge in enumerate(central_context.algorithm_params.num_samples_mixture_bins[0]):
            bin_edge = int(bin_edge)
            if num_samples <= bin_edge:
                selected_bin = i
                break

        user_num_samples_distribution = model.num_samples_distribution[:, selected_bin]
        ...
        statistics['posterior_probabilities'] = posterior_probabilities
        statistics['numerator'] = numerator
        statistics['denominator'] = denominator
        statistics['num_samples_distribution'] = e
    ...
    def process_aggregated_statistics(
        ...
        print('num_samples_distribution', num_samples_distribution.shape, num_samples_distribution)
        ...
        def prevent_zero(tensor, min_val=0, mass_reallocation_percentage=0.01):
            ...
            tensor = torch.where(tensor>min_val, tensor, extra_mass.expand_as(tensor))
            tensor = tensor / torch.sum(tensor, dim=1, keepdim=True) * total_mass
            return tensor
        ...
        if (posterior_probabilities == 0).any():
            ...
            posterior_probabilities = torch.where(posterior_probabilities > 0, posterior_probabilities, extra_mass)
            posterior_probabilities = posterior_probabilities / torch.sum(posterior_probabilities) * total_mass
        ...
        if (denominator == 0).any():
            ...
            if (denominator == 0).any():
                raise AssertionError('prevent zero did not work on denominator')

        if (num_samples_distribution == 0).any():
            modified_num_samples_distribution = prevent_zero(num_samples_distribution, min_val=0.001)
            ...
            while (modified_num_samples_distribution == 0).any():
                ...
                if mass_reallocation_percentage >= 1:
                    raise AssertionError(f'prevent zero did not work on num_samples_distribution: {num_samples_distribution}')
                modified_num_samples_distribution = prevent_zero(num_samples_distribution, min_val=0.001, mass_reallocation_percentage=mass_reallocation_percentage)
        ...
        num_samples_distribution = num_samples_distribution / posterior_probabilities.reshape(-1,1).expand_as(num_samples_distribution)
        ...
        num_samples_distribution = num_samples_distribution / num_samples_distribution.sum(dim=1, keepdim=True)
        if (num_samples_distribution == 0).any():
            num_samples_distribution = prevent_zero(num_samples_distribution, min_val=0.001)
