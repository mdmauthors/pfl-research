...
from typing import Tuple, Optional, TypeVar, Callable, Union, List
...
class PolyaMixtureInitializationAlgorithm(
    ...
    def simulate_one_user(
        ...
        selected_bin = -1
        for i, bin_edge in enumerate(central_context.algorithm_params.num_samples_mixture_bins[component]):
    ...
    def process_aggregated_statistics(
        ...
        if (central_context.current_central_iteration ==
            ...
            k_pmax_coefficient = init_coefficient[torch.arange(init_coefficient.size(0)), k_pmax]

            moment_matching_alpha_k_pmax = p * k_pmax_coefficient.reshape(-1,1)
            ...
            #initial_alphas = [torch.Tensor([alpha_0, alpha_1])]
